...
void calculate_ci1(int n,vecEg2dd &cpsi)
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		A(i, prevI) = (1.0 - lambdai[prevI])*(1 - ti[i])*(1 - ti[i]);
		A(i, i) = lambdai[prevI]*(1 - ti[i])*(1 - ti[i]) + (2.0 - (1.0 + lambdai[i])*ti[i])*ti[i];
		A(i, nextI) = lambdai[i]*ti[i]*ti[i];
		b1[i] = cpsi[i][0];
		b2[i] = cpsi[i][1];
	}
...
int main(int argc, char **argv)
{
	...
	glutAddMenuEntry("openedk-curve", 1);
	glutAddMenuEntry("closedk-curve", 2);
